...
public class MyStringUtils {
	...
	public static String[] loadLine(BufferedReader inStream, String prefix, boolean isList)
		...
		if (!contentString.equals("")) {
			if (!isList) {
				// split nonâˆ’list attribute contents into words
				// using 1âˆ’orâˆ’moreâˆ’whiteâˆ’spaces as separator
				words = contentString.split("\\s+");
				// a nonâˆ’list attribute with contents of more than one word is invalid
				if (words.length != 1)
					throw new FileContentsException(lineTooLongMsg + prefix);
			} else
				// split list attribute contents into words
				// using comma+0âˆ’orâˆ’moreâˆ’whiteâˆ’spaces as separator
				words = contentString.split(",\\s*");
			// the attribute contents are empty
		} else {
			// a nonâˆ’list attribute with empty contents is invalid
			if (!isList)
				throw new FileContentsException(lineTooShortMsg + prefix);
			// a list attibute with empty contents is valid;
			// use a zeroâˆ’length array to store its words
			words = new String[0];
		}
